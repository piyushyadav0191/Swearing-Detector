...
type APISuccess = {
  isCursedAutoCorrect: boolean
  score: number
  flaggedFor: string[] | undefined
}

export const checkCursing = async ({ message }: { message: string }) => {
  try {
    ...
    const res = await fetch('https://autocorrect.auto-correct-cursed-word.workers.dev/', {
      method: 'POST',
