...
import { checkCursing } from '@/actions'
...
const Demo = () => {
  const [message, setMessage] = useState<string>(
    'this is definitely not a cursed word'
  )

  const { data, mutate, isPending, error } = useMutation({
    mutationKey: ['check-cursed'],
    mutationFn: checkCursing,
    onSettled: (data) => {
      ...
      <div className='relative w-full rounded-xl mt-12 bg-gray-900/5 p-4 ring-1 ring-inset ring-gray-900/10 lg:rounded-2xl'>
        <div className='flex justify-between items-center'>
          <div className='flex items-center gap-4'>
            ...
            <p className='break-all'>https://autocorrect.auto-correct-cursed-word.workers.dev/</p>
          </div>
        </div>
            ...
            <div className='flex flex-col items-center text-center'>
              <p className='font-bold'>
                {successData.score > 0.95 ? (
                  <span>
                    🚨🚨😱😱 OH GOD, VERY BIG Cursing DETECTED!! 🚨🚨😱😱{' '}
                  </span>
                ) : successData.score > 0.9 ? (
                  <span>🚨😱 BIG Cursing DETECTED!! 🚨😱 </span>
                ) : successData.score > 0.88 ? (
                  <span>🚨 Cursing DETECTED!! 🚨 </span>
                ) : successData.score >= 0.85 ? (
                  <span>😱 PRETTY SURE THIS IS A Cursing 😱</span>
                ) : successData.score < 0.85 ? (
                  <span>Crispy clean input, no cursing :)) 👍👍</span>
                ) : null}
              </p>
